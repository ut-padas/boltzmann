...
x0 =      0.2347;
x1 =       11.71;
f = @(ev) 9.900000e-20*(a1+b1*(log(ev/x1)).^2)./(1+b1*(log(ev/x1)).^2).*(a0+b0*(log(ev/x0)).^2)./(1+b0*(log(ev/x0)).^2)./(1+c*ev.^d);

% f = @(x) 1e-19*(a1+b1*(log(x/x1)).^2)./(1+b1*(log(x/x1)).^2).*(a0+b0*(log(x/x0)).^2)./(1+b0*(log(x/x0)).^2)./(1+c*x.^d);
x = logspace(-4, 5, 100000);
plot(data(:,1), data(:,2), 'o', x, f(x+1.e-8),'-')
