...
Assuming, Ar species are modeled by $\delta(0)$ distribution, and using the hard sphere collision kernel approximation, the above collusion operator simplifies, to 
\begin{equation}
...
For a given total cross section value, the differential cross section, can be computed as follows\cite{vahedi1995monte}, where $E= \frac{1}{2}mv^2$
\begin{equation}
    \sigma(E,\chi) = \frac{\sigma(\chi)}{4\pi (1 + E \sin^2(\chi/2))\ln(1+E)} 
\end{equation}
...
\begin{itemize}
    ...
    \item $\phi$ : angle between $v_1$ projection to $v_0 \times(v_0 \times e_i)$, $v_0 \times e_i$ plane, and vector $v_0 \times(v_0 \times e_i)$
    \item $\theta$ : angle between $\hat{v_0}$ and $\hat{e_i}$, i.e., $cos\theta = \hat{v_0} \cdot \hat{e_i}$
\end{itemize}
For deterministic solver, $\theta$ can be computed from $v_0$ and $\chi,\phi$ taken as the solid angle. 

The scattered velocity can be decomposed along the orthonormal basis vectors, $\hat{E_0}=\hat{v_{0}}$, $\hat{E_1}= \cfrac{\hat{v_0}\times e_i}{\sin\theta}$, and $\hat{E_2}= \hat{v_0}\times \hat{E_1}$. Therefore, the scattered direction unit vector can be written, 

\begin{equation}
    \hat{v_1} = \cos\chi \hat{E_0} + \sin\chi sin\phi \hat{E_1} + \sin\chi cos\phi \hat{E_2} \label{eq:scatter}
\end{equation}
...
The normally we use excitation threshold of $E_{exc}=11.5eV$.

\subsection{G2 : $e + Ar \rightarrow e + Ar^+ + e$ collision operator}
\begin{itemize}
    \item $v_1$ : velocity of the scattered electron
    \item $v_2$ : velocity of the elected electron from Ar. 
\end{itemize}
\begin{equation}
    C_{G2}(f) = \int_{\chi} \int_{\phi} (f(v_1) + f(v_2) - f(v_0)) \norm{v_0} \sigma_{G2}(\norm{v_0},\omega) \sin\chi d\phi d\chi
\end{equation}

Let $E_{ion}$ be the energy threshold for the ionization reaction, then as in \cite{vahedi1995monte} we split the $E_0-E_{ion}$ equally among scattered and the ejected electron. i.e., $E_1 = 0.5 (E_0-E_{ion})$, $E_2 = 0.5 (E_0-E_{ion})$.

\begin{align}
    \norm{v_1} &= \sqrt{\frac{1}{2}\norm{v_0}^2 - \frac{E_{ion}}{m}}\\
    \norm{v_2} &= \sqrt{\frac{1}{2}\norm{v_0}^2 - \frac{E_{ion}}{m}}
\end{align}
$\hat{v_1}$ derived as in (\ref{eq:scatter}) and $\hat{v_2}$ derived based on the momentum conservation, assuming the momentum change in the Ar atom is negligible. 
\begin{align}
