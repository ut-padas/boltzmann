...
Tg       = 0.5 #eV
vth      = np.sqrt(Tg) * c_gamma
mw       = utils.get_maxwellian_3d(vth=vth, n_scale=1)

lump_mechanism = {
            
            "E + Ar -> E + Ar"     : ["E + Ar -> E + Ar"],
            
            "E + Ar -> E + Ar*(1)" : ["E + Ar -> E + Ar(1S5)", "E + Ar -> E + Ar(1S4)"],
            
            "E + Ar -> E + Ar*(2)" : ["E + Ar -> E + Ar(1S3)", "E + Ar -> E + Ar(1S2)"],
            ...
            "E + Ar*(1) -> E + E + Ar(+)" : ["E + Ar5 -> E + E + Ar(+)", "E + Ar4 -> E + E + Ar(+)"],
            
            "E + Ar*(2) -> E + E + Ar(+)" : ["E + Ar3 -> E + E + Ar(+)", "E + Ar2 -> E + E + Ar(+)"], 
...
num_cs_pts  = 512
for k, lump_r in lump_mechanism.items():
    cs_type    = list()
    ev_min     = list()
    ev_max     = list()
    ...
    cs_weight = np.array([mw(np.sqrt(species_energy[crs_data[r]["species"]]) * c_gamma/vth) for r in lump_r])
    ...
    ev_domain = (max(np.min(ev_min), 1e-6), np.min(ev_max))
    print("cross-section weights for lumped reaction %s\n"%(k), cs_weight)
