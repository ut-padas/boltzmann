...
crs_file                          = "./lxcat_data/eAr_crs.Biagi.3sp2r"
avail_species                     = cross_section.read_available_species(crs_file)
cross_section.CROSS_SECTION_DATA  = cross_section.read_cross_section_data(crs_file)
...
# plt.show()
ev = ev[0::10]
np.savetxt("eAr.dat", np.concatenate([ev.reshape((-1,1)), g0.total_cross_section(ev).reshape((-1,1)), g2.total_cross_section(ev).reshape((-1,1))], axis=1), delimiter="\t", header="energy\telastic\tionization")
