...
class CollisionOpSP():
    ...
    def _LOp_l(self,collision,maxwellian, vth):
        """
        loop based (slow)  + part of the coll op
        \int_r^3 \int_s2 f(v1) dw
        !Note: only works with gauss-maxwell quadrature points
                                ...
                                for lm2_idx,lm2 in enumerate(sph_harm_lm):
                                    for pj in range(num_p):
                                        for lm1_idx,lm1 in enumerate(sph_harm_lm):
                                            i_id = pi*num_sph_harm + lm2_idx
                                            j_id = pj*num_sph_harm + lm1_idx
                                            L_ij[i_id,j_id] += diff_cs * w_factor * gmw[qi] * glw[v_theta_i] * glw_s[theta_i] * WVPhi_q[v_phi_i] * WPhi_q[phi_i]* spec_sp.basis_eval_full(v_abs,v_theta,v_phi, pj,lm1[0],lm1[1]) * (spec_sp.basis_eval_full((vs[0]/V_TH),vs[1],vs[2],pi,lm2[0],lm2[1]) -spec_sp.basis_eval_full(v_abs,  v_theta,v_phi,pi,lm2[0],lm2[1]))
