...
\begin{align*}
	...
	\partial_t {\hat{f}} &=(C+E)\hat{f} + n C_{ee}\of{\hat{f}, \hat{f}} - (u^T W) \hat{f}\\
	&\text{ with constraint } u^T \hat{f} = 1  
\end{align*} where we can derive the $W(\hat{f})$ as follows. 
\begin{align*}
	u^T W \hat{f} = \frac{\partial_t n}{n} = u^T(C+ E + n C_{ee}(\hat{f})) \hat{f}
\end{align*}
For the transient solution we need to evolve the above ODE with the mass constraint $u^T \hat{f}=1$. Let $\hat{f} = f_1 + \bar{f}$ such that $u^T f_1 = 1$ and $u^T \bar{f}=0$.  Let $f_1= \frac{u}{\norm{u}_2^2}$, we can write, 
\begin{align*}
	\partial_t \hat{f} = \partial_t \bar{f} = (C+E)\hat{f} + n C_{ee}(\hat{f}, \hat{f}) - \of{u^T W} \hat{f}
\end{align*}. Let $p = \frac{u}{\norm{u}_2}$ then note that $(I-pp^T)\bar{f} = \bar{f}$. Therefore we can write, 
\begin{align*}
	\partial_t \bar{f} = (I-pp^T)  \partial_t \bar{f} = (I-pp^T) \of{(C+E)\hat{f} + n C_{ee}(\hat{f}, \hat{f}) - \of{u^T W} \hat{f}}
\end{align*} Note that $u^T \bar{f}=0$ by construction. 
Then we evolve the above with $\bar{f}(0) = (I-pp^T) f(0)$

For the steady state solve we can use,\\
