...
\begin{frame}
	...
	\end{columns}
	\begin{itemize}
		\item Completed 1-way integration
	\end{itemize}
	% Current integration
	% \begin{itemize}
...
\begin{frame}
	...
	\begin{itemize}
		\item \textbf{Maxwellian} : $T_e=T_g$ and $k_i^{A} = \int_{\vect{v}} \sigma f^{\text{Maxwellian}} $  (currently used in TPS code)%from Maxwellian EEDF %and $f_M(\vect{v}, t)=M_{T_e}(\vect{v})$ and $k_i^{M} = \mathcal{K}f_M$
		\item \textbf{Static E-field} : Batched \textcolor{orange}{steady-state} 0D-Boltzmann, $k_i^{B} = \int_{\vect{v}} \sigma f^{B} $ 
		\item \textbf{Oscillatory E-field} : Batched \textcolor{orange}{transient} 0D-Boltzmann, $k_i^{C} = \frac{1}{T} \int_{T} \int_{\vect{v}} \sigma f^{C} $ %Batched 0D-Boltzmann solver with oscillatory $\vect{E}$ field with cycle averaged rates, $k_i^{C} = \mathcal{K}f$ with weak-coupling
...
\begin{frame}
	\frametitle{Batched 0D-Boltzmann solver}
	\vspace{-0.25in}
	\begin{center}
		\includegraphics[width=0.8\textwidth]{tps_0d_clusters.png}
	\end{center}
	\begin{itemize}
		\item For $N$ 0D-BTE solves, cluster points by input gas temperature $T_g(\vect{x})$. 
		\item For each cluster EEDF is resolved up to $\varepsilon(\bar{T}_g)$
	\end{itemize}
	\begin{center}
		...
		\begin{tikzpicture}
			\begin{scope}[xshift=0cm]
				...
				\draw[thick, black] (4,-0.2) --node[below, yshift=-0.1cm]{$\varepsilon^{max}_{0}$} (4,0.2);
			...
			\begin{scope}[xshift=5cm]
				\draw[thick, black] (0,0) -- (4,0);
				\draw[thick, black] (4,-0.2) --node[below, yshift=-0.1cm]{$\varepsilon^{max}_{1}$} (4,0.2);
				\draw[thick, black] (0,-0.2) --node[below, yshift=-0.1cm]{$0$} (0,0.2);	
