...
def ode_first_order_linear(collOp:colOpSp.CollisionOpSP, col_list, h_init, maxwellian, vth, ts):
    MVTH  = vth
    MNE   = maxwellian(0) * (np.sqrt(np.pi)**3) * (vth**3)
    MTEMP = collisions.electron_temperature(MVTH)
    COL_OP_TEMP_TOL = 1e-1
    ...
    for t_i, t in enumerate(ts):
        ...
        if(np.abs(temp_t-temp_coll_op)> COL_OP_TEMP_TOL):
            ...
            for g in col_list:
                FOp+=collOp.assemble_mat(g,mw_cop,vth_coll_op)
            t_L.stop()
            print("Collision Operator assembly time (s): ",t_L.seconds)
        ...
        temp_ts[t_i] = temp_t
        print(f"N_e(t={t:.2E})={m0:.10E} maxwellian N_e={MNE:.2E} relative error: {(MNE-m0)/MNE:.8E} t/dt={t/dt_tau:.2E} temperature(K)={temp_t:.5E} maxwellian temp(K)={MTEMP:.5E}")
...
ode_first_order_linear(cf,[col_g0],h_vec,maxwellian,collisions.ELECTRON_THEMAL_VEL,np.linspace(0,1e-10,100))
