...
\end{align*} Projects to $\psi\of{v}$ radial test functions for $v\in V \subset [0,\infty]$.
\begin{align*}
...
\myint_{V} \partial_t F_0 \of{v} v^2 \psi\of{v} \diff{v}  -c  &\Big[F_0\of{v} v^2 \psi\of{v}\Big]_{\partial V}  + c \myint_{V} F_0\of{v} \left(v^2\partial_v \psi\of{v} + 2v\psi\of{v}\right) dv\\
 +& 2c \myint_{V} v f_{1,0} \of{v} \psi\of{v} dv = ... \\
\end{align*}
Similarly for $F_1\of{v}$ we can write, 
\begin{align*}
\myint_{V} \partial_t F_1 \of{v} v^2 \psi\of{v} \diff{v}  +c  \Big[F_1\of{v} v^2 \psi\of{v}\Big]_{\partial V}  - c \myint_{V} F_1\of{v} \left(v^2\partial_v \psi\of{v} + 2v\psi\of{v}\right) dv\\ + 2c \myint_{V} v f_{1,0} \of{v} \psi\of{v} dv = ...
\end{align*}
Let $\phi_{k}\of{v}$ be the trial functions for expansion and $\psi_{p}\of{v}$ be the corresponding test functions. Let $F_0\of{v}=\sum_{k} h_k \phi_k\of{v}$ and  $F_1\of{v}=\sum_{k} g_k \phi_{k}\of{v}$. Notice that $f_{0,0} = \frac{1}{2}(F_{0} + F{1})$, $f_{1,0} = \frac{1}{2}(F_{0} - F{1})$.
Using the above expansion and discretizing the above we can write, 
\begin{align*}
M_{pk} \partial_t \vect{h}  - c\Big[F_0^*\of{v} v^2 \psi\of{v}\Big]_{\partial V} + c (D_{pk} + 2 S_{pk}) \vect{h} + c S_{pk} (\vect{h}-\vect{g})  &= ... \\
M_{pk} \partial_t \vect{g}  + c\Big[F_1^*\of{v} v^2 \psi\of{v}\Big]_{\partial V} - c (D_{pk} + 2 S_{pk}) \vect{g} + c S_{pk} (\vect{h}-\vect{g})  &= ... \\
\end{align*} where the corresponding operators are defined as bellow. 
\begin{align*}
	M_{pk} &= \myint_{V} v^2 \psi_{p}\of{v} \phi_{k}\of{v} dv \\
	D_{pk} &= \myint_{V} v^2 \partial_v \psi_{p}\of{v} \phi_{k}\of{v} dv \\
	S_{pk} &= \myint_{V} v \psi_{p}\of{v} \phi_{k}\of{v} dv \\
...
\begin{align*}
M_{pk} \partial_t \vect{h}  - c\Big[F_0^*\of{v} v^2 \psi\of{v}\Big]_{\partial V} + c (D_{pk} + 2 S_{pk}) \vect{h} + c S_{pk} (\vect{h}-\vect{g})  &= C^{p00}_{k00} \frac{1}{2} (\vect{h} + \vect{g}) + C^{p00}_{k10} \frac{1}{2} (\vect{h} - \vect{g})  \\
M_{pk} \partial_t \vect{g}  + c\Big[F_1^*\of{v} v^2 \psi\of{v}\Big]_{\partial V} - c (D_{pk} + 2 S_{pk}) \vect{g} + c S_{pk} (\vect{h}-\vect{g})  &= C^{p10}_{k00} \frac{1}{2} (\vect{h} + \vect{g}) + C^{p10}_{k10} \frac{1}{2} (\vect{h} - \vect{g}) \\
