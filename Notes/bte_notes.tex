...
Let $\hat{f}$ be the normalized distribution function. \\
\textbf{Algorithm 1:} We use the newton step with linearized $C_{ee}$ term. 
\begin{align*}
	&R(\hat{f}^{n}) =
	 \begin{bmatrix}
		(C + E)\hat{f}^{n} - (u^T W(\hat{f}^{n})) \hat{f}^{n} + C_{ee}(\hat{f}^{n-1}) \hat{f}^{n}\\
		u^T \hat{f} -1
	\end{bmatrix} =0 \\
	&J(\hat{f}^{n}) =
	\begin{bmatrix}
		(C + E) - 2(u^T W)\hat{f}^{n} + C_{ee}(\hat{f}^{n-1})\\
		u^T -1
	\end{bmatrix} =0 \\
...
\begin{align*}
	& \partial_t \hat{f} = (C + E) \hat{f}^{n} - (u^T W(\hat{f}^{n})) \hat{f}^{n} + C_{ee} (\hat{f}^{n}, \hat{f}^{n}) =0 \\
	& \hat{f}^{n+1} = (C+E)^{-1} \of{-C_{ee} (\hat{f}^{n}, \hat{f}^{n}) +  (u^T W(\hat{f}^{n})) \hat{f}^{n} }
\end{align*}

\textbf{Algorithm 3:} Semi-implicit on the coulomb collision operator. \\
\begin{align*}
	& \partial_t \hat{f} = (C + E) \hat{f}^{n} - (u^T W(\hat{f}^{n})) \hat{f}^{n} + C_{ee} (\hat{f}^{n}, \hat{f}^{n}) =0 \\
	& \hat{f}^{n+1} = (C+E + C_{ee}(\hat{f}^{n}))^{-1} \of{ (u^T W(\hat{f}^{n})) \hat{f}^{n} }
\end{align*}
