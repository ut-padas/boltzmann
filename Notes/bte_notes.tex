...
\item Solve \eqref{eq:shiftedColOp} from $t_0$ to $t_0 + \Delta t$
...
\begin{align*}
	...
	e + Ar^{+} + e &\rightarrow e + Ar
\end{align*}
Denote momenta of particles on one side of the reaction as $\vect{p}_e$ and $\vect{p}_0$ and on the other side as $\vect{p}_+$, $\vect{p}_1$, and $\vect{p}_2$. Their total momenta, $\vect{K}_{0,e}$ and $\vect{K}_{+,1,2}$, and energies, $E_{0,e}$ and $E_{+,1,2}$, can be expressed as:
\begin{align*}
\vect{K}_{0,e} &= \vect{p}_0 + \vect{p}_e
\\
\vect{K}_{+,1,2} &= \vect{p}_+ + \vect{p}_1 + \vect{p}_2
\\
E_{0,e} &= \frac{\vect{p}_0^2}{2(m+M)} + \frac{\vect{p}_e^2}{2m} 
\\
&= \frac{\vect{K}_{0,e}^2}{2(2m+M)} + u_{0,e}
\\
...
\end{align*}
where $m$ and $M$ are masses of an electron and $Ar^+$; $\vect{p}_{12}$ is the total momentum of two electrons; $\vect{k}_{0,e}$, $\vect{k}_{1,2}$, and $\vect{k}_{+,12}$ are the relative momenta of Ar-electron, electron-electron, and Ar$^+$-pair of electrons systems, and $\mu_{0,e}$, $\mu_{1,2}$, and $\mu_{+,12}$ are the corresponding reduced masses:
\begin{align*}
u_{0,e} &= \frac{\vect{k}_{0,e}^2}{2\mu_{0,e}}, \quad \mu_{0,e} = \frac{m(m+M)}{(2m+M)}
\\
u_{1,2} & = \frac{\vect{k}_{1,2}^2}{2\mu_{1,2}}, \quad \mu_{1,2} = \frac{m}{2}
\\
u_{+,12} & = \frac{\vect{k}_{+,12}^2}{2\mu_{+,12}}, \quad \mu_{+,12} = \frac{2Mm}{2m+M}
\\
\vect{k}_{0,e} &= \frac{m \vect{p}_0 - (M+m) \vect{p}_e}{2m+M} 
\\
\vect{k}_{1,2} &= \frac{m \vect{p}_1 - m \vect{p}_2}{2m} = \frac{\vect{p}_1 - \vect{p}_2}{2}
\\
\vect{p}_{12} &= \vect{p}_1 + \vect{p}_2
\\
\vect{k}_{+,12} &= \frac{2m \vect{p}_+ - M \vect{p}_{12}}{2m+M}
\end{align*}

 The conservation of energy becomes:
\begin{align*}
u_{0,e} = u_{+,12} + u_{1,2} + \Delta E_i
\end{align*}

In case of the ionization process, for given incident momenta $\vect{p}_e$ and $\vect{p}_0$ (such that $u_{0,e} > \Delta E_i$) a collision can be specified by the magnitude of relative energy of electrons $u_{1,2} < u_{0,e} -\Delta E_i$ and directions of relative momenta $\hat{\vect{k}}_{1,2}$ and $\hat{\vect{k}}_{+,12}$. In case of the recombination process, for given incident momenta $\vect{p}_+$, $\vect{p}_1$, and $\vect{p}_2$ a collision can be specified by just direction of the relative momentum $\hat{\vect{k}}_{0,e}$.
...
\delta\of{\vect{K}_{0,e} - \vect{K}_{+,1,2}} 
\delta\of{E_{0,e} - E_{+,1,2} - \Delta E_i}
\end{multline*}
...
It is convenient to integrate out delta functions by changing variable from $(\vect{p}_+, \vect{p}_1, \vect{p}_2)$ to $(\vect{K}_{+,1,2}, \vect{k}_{+,12}, \vect{k}_{1,2})$ for ionization collisions, and from $(\vect{p}_0, \vect{p}_e)$ to $(\vect{K}_{0,e}, \vect{k}_{0,e})$ for recombination collisions: 
\begin{align*}
d^3 \vect{p}_+ d^3 \vect{p}_1 d^3 \vect{p}_2
...
\\
d^3 \vect{p}_0 d^3 \vect{p}_e 
&= d^3 \vect{K}_{0,e} d^3 \vect{k}_{0,e}
\\
&= d^3 \vect{K}_{0,e} \mu_{0,e} k_{0,e} d u_{0,e} d \Omega\of{\hat{\vect{k}}_{0,e}}
\end{align*}
...
= (2\pi h)^2 
\int_{R^3} \int_{R^3} 
\int_{S^2} \int_{0}^{u_{0,e} - \Delta E_i} \int_{S^2} 
d^3 \vect{p}_0 d^3 \vect{p}_e
...
\end{multline*}
where integration with respect to $\vect{p}_0$ and $\vect{p}_e$ are limited by the condition $u_{0,e} > \Delta E_i$, and
\begin{multline*}
\left< C_r, \psi \right>  =
(2\pi h)^2 
...
k^{post}_{+,12} &= \sqrt{ 2 \mu_{+,12} (u_{0,e} - u_{1,2} - \Delta E_i)} \\
\vect{p}_1 &= k^{post}_{1,2} \hat{\vect{k}}_{1,2} - \frac12 k^{post}_{+,12} \hat{\vect{k}}_{+,12} + \frac{m}{M+2m} (\vect{p}_e + \vect{p}_0) \\
k^{post}_{0,e} &= \sqrt{ 2 \mu_{0,e} (u_{1,2} + u_{+,12} + \Delta E_i)} \\
\vect{p}^{post}_e &= - k^{post}_{0,e} \hat{\vect{k}}_{0,e} + \frac{m}{2m + M} (\vect{p}_+ + \vect{p}_1 + \vect{p}_2)
\end{align*}
...
\begin{align*}
\sigma_{i,CM} \of{ \vect{k}_{0,e} \rightarrow u_{1,2}, \hat{\vect{k}}_{1,2}, \hat{\vect{k}}_{+,12}} 
...
Substituting back
\begin{align*}
\sigma_{i,CM}\of{\vect{k}_{0,e} \rightarrow \hat{\vect{k}}_{+,12}, \hat{\vect{k}}_{1,2}, u_{1,2}} 
=
\left( 2 \pi \planck \right)^2 
\mu_{0,e} \mu_{1,2} \mu_{+,12} \frac{k^{post}_{+,12}}{k_{e,0}} k^{post}_{1,2}
| T\of{\vect{p}_0^{post}, \vect{p}_e^{post} \leftrightarrow \vect{p}_+, \vect{p}_1, \vect{p}_2} |^2
\end{align*}
Thus
\begin{align*}
| T\of{\vect{p}_0^{post}, \vect{p}_e^{post} \leftrightarrow \vect{p}_+, \vect{p}_1, \vect{p}_2} |^2
= \frac{ k_{e,0} \sigma_{i,CM}\of{\vect{k}_{0,e} \rightarrow \hat{\vect{k}}_{+,12}, \hat{\vect{k}}_{1,2}, u_{1,2}} }
{ \left( 2 \pi \planck \right)^2 
\mu_{0,e} \mu_{1,2} \mu_{+,12} k^{post}_{+,12} k^{post}_{1,2} }
\end{align*}

Experimental data typically contains only the dependence of the total scattering cross section on the incident energy $\sigma^{total}_{i,CM} = \sigma^{total}_{i,CM}\of{k_{0,e}}$. Assuming the differential cross section is uniform in all directions and scattering energies: 
\begin{align*}
\sigma_{i,CM}\of{\vect{k}_{0,e} \rightarrow \hat{\vect{k}}_{+,12}, \hat{\vect{k}}_{1,2}, u_{1,2}}
= \frac{ \sigma^{total}_{i,CM}\of{k_{0,e}} }
{ (u_{0,e}-\Delta E_i) (4\pi)^2 }
\end{align*}
Finally, neglecting the difference between the center-of-mass frame of reference and laboratory one (in which target is stationary) due to the extreme mass ratio between the target (Ar or Ar$^+$) and electrons, i.e. $\sigma^{total}_{i,CM}\of{k_{0,e}} \approx \sigma^{total}_{i}\of{k_{0,e}}$:
\begin{align*}
| T\of{\vect{p}_0, \vect{p}_e \leftrightarrow \vect{p}_+^{post}, \vect{p}_1^{post}, \vect{p}_2^{post}} |^2
&= \frac{ k_{e,0} \sigma^{total}_{i}\of{k_{0,e}} }
{ (4\pi)^2 \left( 2 \pi \planck \right)^2 (u_{0,e}-\Delta E_i) 
\mu_{0,e} \mu_{1,2} \mu_{+,12} k^{post}_{+,12} k^{post}_{1,2} }
\\
| T\of{\vect{p}_0^{post}, \vect{p}_e^{post} \leftrightarrow \vect{p}_+, \vect{p}_1, \vect{p}_2} |^2
&= \frac{ k_{e,0}^{post} \sigma^{total}_{i}\of{k_{0,e}^{post}} }
{ (4\pi)^2 \left( 2 \pi \planck \right)^2 (u_{+,12} + u_{1,2}) 
\mu_{0,e} \mu_{1,2} \mu_{+,12} k_{+,12} k_{1,2} }
\end{align*}
...
\left( \frac12 \psi\of{\vect{p}_e^{post}}
- \psi\of{\vect{p}_1} \right) h^3 f_+\of{\vect{p}_+} f_e\of{\vect{p}_1}  f_e\of{\vect{p}_2} 
\frac{ \left( k_{e,0}^{post} \right)^2 \sigma^{total}_{i}\of{k_{0,e}^{post}} }
{ (4\pi)^2 (u_{+,12} + u_{1,2}) 
...
n_0
\int_{R^3} 
\int_{S^2} \int_{S^2} \int_{0}^{u_{0,e} - \Delta E_i} 
...
\\
\times B_r\of{\vect{v}_1, \vect{v}_2, \hat{\vect{k}}_{0,e}}
...
&=
\frac{ m^3 k_{e,0} \sigma^{total}_{i}\of{k_{0,e}} }
{ (4\pi)^2 (u_{0,e}-\Delta E_i) 
\mu_{0,e} }
\\
B_r\of{\vect{v}_1, \vect{v}_2, \hat{\vect{k}}_{0,e}} &=
\frac{ m^6 h^3 \left( k_{e,0}^{post} \right)^2 \sigma^{total}_{i}\of{k_{0,e}^{post}} }
{ (4\pi)^2 (u_{+,12} + u_{1,2}) 
\mu_{1,2} \mu_{+,12} k_{+,12} k_{1,2} }
\end{align*}
