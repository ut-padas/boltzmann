...
\begin{align}
	...
	&k_{i} = F(t_n + \hat{c}_i \Delta t, U^n + \Delta t \sum_{j=1}^{i-1} \hat{A}_{ij} F\of{U^j}) + G(t_n + c_i \Delta t, U^n + \Delta t \sum_{j=1}^{s} A_{ij} G\of{U^j}) \\%\Delta t \sum_{j=1}^{i} A_{ij} G\of{U^{j}} \\
...
\end{align} where $\vect{x}_{AP}$ and $\vect{v}_{AP}$ denotes the solution at arrival point (AP) computed from semi-Lagrangian (SL) advection. Let $f^{n}(x,v,\vtheta)$ be the solution at $t=t_n$. Then the solution at $f(\vect{x}_{AP}, \vect{v}_{AP})$ can be written as \eqref{eq:sl_adv}. %Let $\{x_i\}_{i=1}^{N_p}$, $\{v_i\}_{i=1}^{N_r}$, and $\{\vtheta_i\}_{i=1}^{N_\theta}$ be the grid used for SL advection.
\begin{align}
...
%$\vect{v}_{AP} = \vect{v}_{DP} + \frac{q_e E}{m_e} \Delta t$ where $\vect{v}_{DP}$ denotes the departure point.  
%\begin{align}
%	\vect{v}_{DP} (t) = \int_{R^3} \vect{v} f(t, \vect{v}) d \vect{v}
%\end{align} For $\vect{x}$ advection, we use the diagolization of A where, $A=Q_A \Lambda_A Q_A^{-1}$. Let $h = Q_{A}^{-1}f$ then we can write the Lagrangian advection \eqref{eq:bte_x_adv_q1}.
%\begin{align}
%h(\Delta t,\vect{x}_{AP}) = h_0(\vect{x}_{AP} - \Lambda\cos(\vect{\vtheta}) \Delta t) \label{eq:bte_x_adv_q1}
%\end{align}. For the v-space advection we can write the Galerkin projection operators as \eqref{eq:projection}
%\begin{align*}
%	P = M^{-1} K \text{ where } K_{klm}^{pqs} = \int_{\vect{V}} B_k\of{v_r} Y_{lm}\of{\vtheta, \vphi} B_p\of{v_r^\prime} Y_{qs}\of{\vtheta^\prime, \vphi^\prime} d \vect{v} \label{eq:projection}
%\end{align*} where $\vect{v}= \{v_r, \vtheta, \vphi\}$ and $\vect{v}_{AP}=\{v_r^\prime, \vtheta^\prime, \vphi^\prime\}$.
