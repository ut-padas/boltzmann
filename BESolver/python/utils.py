...
def assemble_moment_ops_ords(spec_sp: spec_spherical.SpectralExpansionSpherical, num_vt, NUM_Q_VR, NUM_Q_VT, NUM_Q_VP, mf, scale=1.0):
    ...
    Vqr = spec_sp.Vq_r(vg[0], l=0, scale=1)
    Vqt = np.zeros(tuple([num_vt]) + vg[0].shape)
    
    for i in range(num_vt):
        Vqt[i, vg[1]==vt_q[i]] = 1.0 
    
    Vq  = np.einsum("iklm,jklm->ijklm", Vqr, Vqt).reshape(tuple([num_p * num_vt])+vg[0].shape)

    num_m         = len(mf)
    mf_eval       = np.zeros(tuple([num_m]) + vg[0].shape)

    for i in range(num_m):
       mf_eval[i] = wx * mf[i](vg[0], vg[1], vg[2])
...
def gmres_pcr(xp, A, b, x0=None, atol=1e-20, rtol=1e-10, maxiter=None, restart=None, M=None, callback=None):
    """
...
def gmres_pcl(xp, A, b, x0=None, atol=1e-20, rtol=1e-10, maxiter=None, restart=None, M=None, callback=None):
    """
    Standard GMRES implementation without restarts
    :param xp: Array module (numpy or cupy)
    :param A: Linear operator (matrix or object with matvec method)
...
def gmres(xp, A, b, x0=None, atol=1e-20, rtol=1e-10, maxiter=None, restart=None, M=None, callback=None, pc_left=False):
    if pc_left==True:
        return gmres_pcl(xp, A, b, x0, atol, rtol, maxiter, restart, M, callback)
    else:
