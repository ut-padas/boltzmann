...
%\item Solution as a perturbed Maxwellian $M\of{v}$:
...
%\begin{itemize}
%\item Electron density function $f = f(\vect{x}, \vect{v}, t)$
%\end{itemize}
%\small
%\begin{align*}
...
%\begin{align*}
%Q_a(f) &= n_0\int_{S^2} v \sigma_a(v,\omega) 
%\left( f(v^\prime) - f(v) \right) d\omega \text{ where } Q(f) = \sum_{a} Q_a(f)
%\end{align*}
%% \item \textbf{Main challenge}: 6+1 dimensions
%% \item \textbf{Idea}: FEM in $\vect{x}$, spectral in $\vect{v}$
