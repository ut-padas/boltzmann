...
\begin{align*}
	P^{m}_l\of{v} = 4\pi \int_{0}^{v} {v^\prime}^m f^b_l(v^\prime) dv^\prime\\
	Q^{m}_l\of{v} = 4\pi \int_{v}^{\infty} {v^\prime}^m f^b_l(v^\prime) dv^\prime \\
...
\begin{align*}
	...
	&C^{10}_{pk} = \Gamma_a \int (1+\alpha) \of{v Q^0_1\of{v} S_k\of{x}} T_p\of{x} dv\\
	&-\Gamma_a \int \Big(\frac{1}{3}\of{(2\alpha -1 ) P^3_1\of{v} - (1 + \alpha) v^3 Q^0_1\of{v}}S_k\of{x} + \frac{1}{5v}\of{P^5_1\of{v} + v^5 Q^0_1\of{v}}\partial_v S_k\of{x} \Big) \partial_v \of{T_p\of{x}/v} dv \\
...
\begin{align*}
	P^{m}_l\of{v} = P_{mk} f^{b}_{kl} \text{ where } P_{mk} = 4\pi (v_{th})^{m+1}\int_{0}^{x} {x^\prime}^m S_k(x^\prime) dx^\prime \\
	Q^{m}_l\of{v} = Q_{mk} f^{b}_{kl} \text{ where } Q_{mk} = 4\pi (v_{th})^{m+1}\int_{x}^{\infty} {x^\prime}^m S_k(x^\prime) dx^\prime \\
\end{align*}. For electron-electron interaction the derived collision operator is quadratic in $f$ (i.e., $C_{ee} : (f, f) \rightarrow [C_{ee}\of{f}] f$). For the steady state solve, we would need the Jacobian of $C_ee$ w.r.t. $f$. We can write the Jacobian as, 
\begin{align*}
